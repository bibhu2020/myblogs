"""Entry point: python -m maintenance_agent"""
import sys
import time

# Support: python -m maintenance_agent schedule '0 0 1 * *'
if len(sys.argv) >= 2 and sys.argv[1] == "schedule":
    cron_expr = sys.argv[2] if len(sys.argv) >= 3 else ""
    if not cron_expr:
        print("Error: 'schedule' requires a cron expression, e.g.: python -m maintenance_agent schedule '0 0 1 * *'")
        sys.exit(1)
    ...
    if not croniter.is_valid(cron_expr):
        ...
        print('   Examples: "0 0 1 * *"  (1st of each month at midnight UTC)')
        sys.exit(1)

    from .main import run_maintenance

    print("Meridian Maintenance Agent scheduler started")
    ...
    try:
        ...
        print(f"\nMaintenance agent failed: {e}", file=sys.stderr)
        sys.exit(1)
