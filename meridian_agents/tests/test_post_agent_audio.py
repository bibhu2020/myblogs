...
from meridian_agents.post_agent.nodes import audio as audio_mod
from meridian_agents.post_agent.nodes.audio import _upload_audio, generate_audio_node


class TestUploadAudio:
    def test_returns_the_uploaded_url(self):
        with patch.object(audio_mod, "make_agent_jwt", return_value="jwt-token"), \
             patch("meridian_agents.post_agent.nodes.audio.requests.post") as mock_post:
            mock_post.return_value = MagicMock(ok=True, json=lambda: {"url": "/uploads/x.mp3"})
            url = _upload_audio(b"data", "My Post Title", "https://server")
        assert url == "/uploads/x.mp3"

    def test_sends_the_filename_as_a_query_param_when_provided(self):
        with patch.object(audio_mod, "make_agent_jwt", return_value="jwt-token"), \
             patch("meridian_agents.post_agent.nodes.audio.requests.post") as mock_post:
            mock_post.return_value = MagicMock(ok=True, json=lambda: {"url": "/uploads/post_9.mp3"})
            _upload_audio(b"data", "alt", "https://server", filename="post_9")
        _, kwargs = mock_post.call_args
        assert kwargs["params"] == {"filename": "post_9"}
        assert kwargs["files"]["file"][0] == "post_9.mp3"
...
class TestGenerateAudioNode:
    ...
    def test_synthesizes_and_uploads_narration(self, monkeypatch):
        ...
        with patch("meridian_agents.post_agent.nodes.audio.requests.post") as mock_post, \
            ...
            result = generate_audio_node(self.STATE)
        ...
        assert tts_call.kwargs["json"]["style"] == "blog"
    ...
    def test_names_the_mp3_by_the_post_id_once_it_exists(self, monkeypatch):
        monkeypatch.setenv("SERVER_BASE", "https://server")
        state = {**self.STATE, "pending_post_id": 42}
        with patch("meridian_agents.post_agent.nodes.audio.requests.post") as mock_post, \
             patch.object(audio_mod, "_upload_audio", return_value="https://server/uploads/post_42.mp3") as mock_upload:
            mock_post.return_value = MagicMock(content=b"x" * 2048, raise_for_status=lambda: None)
            generate_audio_node(state)
        assert mock_upload.call_args[0][3] == "post_42"

    def test_uses_no_filename_when_post_id_is_unknown(self, monkeypatch):
        monkeypatch.setenv("SERVER_BASE", "https://server")
        with patch("meridian_agents.post_agent.nodes.audio.requests.post") as mock_post, \
             patch.object(audio_mod, "_upload_audio", return_value="https://server/uploads/x.mp3") as mock_upload:
            mock_post.return_value = MagicMock(content=b"x" * 2048, raise_for_status=lambda: None)
            generate_audio_node(self.STATE)
        assert mock_upload.call_args[0][3] is None

    def test_skips_gracefully_when_tts_request_fails(self, monkeypatch):
        monkeypatch.setenv("SERVER_BASE", "https://server")
        with patch("meridian_agents.post_agent.nodes.audio.requests.post", side_effect=Exception("tts down")):
            result = generate_audio_node(self.STATE)
    ...
    def test_skips_when_content_has_no_text(self):
        result = generate_audio_node({**self.STATE, "final_content": "<img src='x.jpg'/>"})
        assert result["audio_url"] is None
