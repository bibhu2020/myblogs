...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="showForm=!showForm" class="bg-primary-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-primary-700 transition-colors">+ Create Guest User</button>
    </div>
    ...
    <div v-if="showForm" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <input v-model="form.name" type="text" placeholder="Full Name *" required class="px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400" />
        <input v-model="form.email" type="email" placeholder="Email *" required class="px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400" />
        <input v-model="form.password" type="password" placeholder="Password *" required class="px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400" />
        <select v-model="form.role" class="px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 bg-white">
          <option value="guest">Guest Author</option>
          <option value="admin">Admin</option>
        </select>
        <textarea v-model="form.bio" rows="2" placeholder="Bio (optional)" class="sm:col-span-2 px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 resize-none"></textarea>
      </div>
      <div class="flex gap-3 mt-4">
        <button @click="save" class="bg-primary-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-primary-700 transition-colors">Create User</button>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0">
                  <span class="text-primary-600 font-bold text-sm">{{ user.name.charAt(0) }}</span>
                </div>
                <div>
            ...
            <td class="px-6 py-4 hidden sm:table-cell">
              <span :class="user.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-primary-100 text-primary-700'" class="px-2.5 py-1 rounded-full text-xs font-semibold capitalize">{{ user.role }}</span>
            </td>
