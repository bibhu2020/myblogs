    ...
    <section v-if="blog.featured.length" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="relative overflow-hidden rounded-3xl aspect-[4/3] lg:aspect-[16/10]">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-8">
              ...
              <div class="flex items-center gap-3 text-gray-300 text-xs">
                <div class="w-7 h-7 bg-primary-500 rounded-full flex items-center justify-center"><span class="text-white font-bold">{{ (blog.featured[0].authorName||'A').charAt(0) }}</span></div>
        ...
        <div class="lg:col-span-5 flex flex-col gap-4">
          <RouterLink v-for="post in blog.featured.slice(1, 4)" :key="post.id" :to="`/blog/${post.slug}`" class="group flex gap-4 bg-gray-50 rounded-2xl p-4 hover:bg-primary-50 transition-colors">
            ...
            <div class="flex-1 min-w-0">
              ...
              <h3 class="font-bold text-gray-900 text-sm line-clamp-2 group-hover:text-primary-600 transition-colors" style="font-family:'Playfair Display',serif">{{ post.title }}</h3>
          ...
          <div class="bg-gradient-to-br from-primary-600 to-primary-800 rounded-2xl p-6 text-white">
            ...
            <p class="text-primary-100 text-sm mb-4">Get the latest tech insights and travel stories delivered to your inbox.</p>
            <div class="flex gap-2">
              <input type="email" placeholder="your@email.com" class="flex-1 px-3 py-2 rounded-lg bg-white/20 text-white placeholder-primary-200 text-sm border border-white/30 focus:outline-none focus:border-white" />
    ...
    <section v-if="techPosts.length" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 bg-primary-100 rounded-xl flex items-center justify-center text-xl">💻</div>
          <div>
        ...
        <RouterLink to="/category/technology" class="text-primary-600 text-sm font-semibold hover:underline flex items-center gap-1">
