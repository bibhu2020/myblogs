...
<!-- HOLIDAY-BANNER-B-END -->
  ...
  <nav :class="layout.variant === 'b'
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16 items-center gap-6">
        ...
        <div class="hidden md:flex items-center gap-1 flex-1">
          ...
          <RouterLink to="/news"
            class="px-3 py-2 text-sm font-medium transition-colors rounded-lg"
            :class="layout.variant === 'b'
              ? (route.path === '/news' ? 'text-violet-400 bg-violet-950' : 'text-slate-400 hover:text-violet-400 hover:bg-slate-900')
      ...
      <!-- Mobile menu — main nav is in the bottom bar; this exposes Topics, Search, Admin -->
      <div v-if="mobileOpen"
        class="md:hidden py-3 border-t space-y-1"
        :class="layout.variant === 'b' ? 'border-slate-800' : 'border-gray-100'"
      >
        <!-- Search -->
        <form @submit="submitSearch" class="px-4 pb-2">
          <div class="relative">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none"
              :class="layout.variant === 'b' ? 'text-slate-500' : 'text-gray-400'"
              fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              :class="layout.variant === 'b'
                ? 'bg-slate-900 border border-slate-700 text-slate-200 placeholder-slate-500 focus:border-violet-500'
                : 'border border-gray-200 bg-gray-50 focus:border-primary-400'"
            />
          </div>
        </form>
        <!-- Topics -->
        <div class="px-4 pt-1 pb-1 text-xs font-bold uppercase tracking-wider"
        ...
        <RouterLink v-for="cat in blog.categories" :key="cat.id"
          ...
          class="flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg"
